...
def _merged_profile_defaults(
    symbol: str, market_type: str, execution_profile: str
) -> dict[str, float | int | bool | str]:
    defaults = dict(EXECUTION_PROFILE_DEFAULTS[execution_profile])
    normalized_symbol = (
        _normalize_equity_symbol(symbol)
...
def get_settings() -> Settings:
    ...
    profile_defaults = _merged_profile_defaults(
        symbol=symbol, market_type=market_type, execution_profile=execution_profile
    )
    ...
    execution_settings = ExecutionSettings(
        capital_per_trade=_get_env_float(
            "CAPITAL_PER_TRADE", float(profile_defaults["capital_per_trade"])
        ),
        account_balance=_get_env_float("ACCOUNT_BALANCE", 200000.0),
        risk_per_trade_pct=_get_env_float(
            "RISK_PER_TRADE_PCT", float(profile_defaults["risk_per_trade_pct"])
        ),
        max_capital_exposure=_get_env_float(
            "MAX_CAPITAL_EXPOSURE", float(profile_defaults["max_capital_exposure"])
        ),
        ...
        max_premium=_get_env_float(
            "MAX_PREMIUM", float(profile_defaults["max_premium"])
        ),
        ...
        range_compression_threshold_pct=_get_env_float(
            "RANGE_COMPRESSION_THRESHOLD_PCT", 0.012
        ),
        atr_stop_multiplier=_get_env_float("ATR_STOP_MULTIPLIER", 1.5),
        fixed_trail_step_pct=_get_env_float("FIXED_TRAIL_STEP_PCT", 0.08),
        volume_spike_multiplier=_get_env_float(
            "VOLUME_SPIKE_MULTIPLIER",
            float(profile_defaults["volume_spike_multiplier"]),
        ),
        max_daily_loss=_get_env_float(
            "MAX_DAILY_LOSS", float(profile_defaults["max_daily_loss"])
        ),
        max_drawdown=_get_env_float(
            "MAX_DRAWDOWN", float(profile_defaults["max_drawdown"])
        ),
        max_trades_per_day=int(
            os.getenv("MAX_TRADES_PER_DAY", str(profile_defaults["max_trades_per_day"]))
        ),
        cooldown_after_two_losses=int(
            os.getenv(
                "COOLDOWN_AFTER_TWO_LOSSES",
                str(profile_defaults["cooldown_after_two_losses"]),
            )
        ),
        stop_after_three_losses=int(os.getenv("STOP_AFTER_THREE_LOSSES", "3")),
        partial_profit_rr=_get_env_float(
            "PARTIAL_PROFIT_RR", float(profile_defaults["partial_profit_rr"])
        ),
        partial_profit_size_pct=_get_env_float(
            "PARTIAL_PROFIT_SIZE_PCT",
            float(profile_defaults["partial_profit_size_pct"]),
        ),
        time_exit_minutes=int(
            os.getenv("TIME_EXIT_MINUTES", str(profile_defaults["time_exit_minutes"]))
        ),
        no_movement_threshold_pct=_get_env_float("NO_MOVEMENT_THRESHOLD_PCT", 0.05),
        hard_flatten_minutes_before_close=int(
            os.getenv("HARD_FLATTEN_MINUTES_BEFORE_CLOSE", "20")
        ),
        slippage_entry_factor=_get_env_float(
            "SLIPPAGE_ENTRY_FACTOR", float(profile_defaults["slippage_entry_factor"])
        ),
        slippage_entry_factor_high=_get_env_float(
            "SLIPPAGE_ENTRY_FACTOR_HIGH",
            float(profile_defaults["slippage_entry_factor_high"]),
        ),
        max_entry_slippage_pct=_get_env_float(
            "MAX_ENTRY_SLIPPAGE_PCT", float(profile_defaults["max_entry_slippage_pct"])
        ),
        weekly_expiry_cutoff_hour=int(os.getenv("WEEKLY_EXPIRY_CUTOFF_HOUR", "13")),
        kill_switch=_get_env_bool("KILL_SWITCH", False),
        trading_window_morning_start=os.getenv(
            "TRADING_WINDOW_MORNING_START", "09:30"
        ).strip(),
        trading_window_morning_end=os.getenv(
            "TRADING_WINDOW_MORNING_END", "11:30"
        ).strip(),
        trading_window_afternoon_start=os.getenv(
            "TRADING_WINDOW_AFTERNOON_START", "13:30"
        ).strip(),
        trading_window_afternoon_end=os.getenv(
            "TRADING_WINDOW_AFTERNOON_END", "14:30"
        ).strip(),
        stop_loss_mode=os.getenv("STOP_LOSS_MODE", "ATR").strip().upper() or "ATR",
        trailing_mode=os.getenv("TRAILING_MODE", "PREVIOUS_CANDLE").strip().upper()
        or "PREVIOUS_CANDLE",
        min_stop_loss_pct=_get_env_float(
            "MIN_STOP_LOSS_PCT", float(profile_defaults["min_stop_loss_pct"])
        ),
        max_stop_loss_pct=_get_env_float(
            "MAX_STOP_LOSS_PCT", float(profile_defaults["max_stop_loss_pct"])
        ),
        trailing_rr_lock_step=_get_env_float("TRAILING_RR_LOCK_STEP", 0.75),
        trailing_buffer_pct=_get_env_float(
            "TRAILING_BUFFER_PCT", float(profile_defaults["trailing_buffer_pct"])
        ),
        drawdown_size_reduction_start_pct=_get_env_float(
            "DRAWDOWN_SIZE_REDUCTION_START_PCT", 0.5
        ),
        drawdown_size_reduction_factor=_get_env_float(
            "DRAWDOWN_SIZE_REDUCTION_FACTOR", 0.5
        ),
        ...
        enable_higher_timeframe_trend_filter=_get_env_bool(
            "ENABLE_HIGHER_TIMEFRAME_TREND_FILTER", True
        ),
        ...
        filter_score_threshold=_get_env_float(
            "FILTER_SCORE_THRESHOLD", float(profile_defaults["filter_score_threshold"])
        ),
        filter_score_trending_bonus=_get_env_float(
            "FILTER_SCORE_TRENDING_BONUS",
            float(profile_defaults["filter_score_trending_bonus"]),
        ),
        filter_score_volatile_bonus=_get_env_float(
            "FILTER_SCORE_VOLATILE_BONUS",
            float(profile_defaults["filter_score_volatile_bonus"]),
        ),
        filter_score_sideways_penalty=_get_env_float(
            "FILTER_SCORE_SIDEWAYS_PENALTY",
            float(profile_defaults["filter_score_sideways_penalty"]),
        ),
        filter_confidence_bonus=_get_env_float(
            "FILTER_CONFIDENCE_BONUS",
            float(profile_defaults["filter_confidence_bonus"]),
        ),
        filter_sideways_weight=_get_env_float(
            "FILTER_SIDEWAYS_WEIGHT", float(profile_defaults["filter_sideways_weight"])
        ),
        filter_ema_weight=_get_env_float(
            "FILTER_EMA_WEIGHT", float(profile_defaults["filter_ema_weight"])
        ),
        filter_volatility_weight=_get_env_float(
            "FILTER_VOLATILITY_WEIGHT",
            float(profile_defaults["filter_volatility_weight"]),
        ),
        filter_vwap_weight=_get_env_float(
            "FILTER_VWAP_WEIGHT", float(profile_defaults["filter_vwap_weight"])
        ),
        filter_volume_weight=_get_env_float(
            "FILTER_VOLUME_WEIGHT", float(profile_defaults["filter_volume_weight"])
        ),
        filter_higher_tf_weight=_get_env_float(
            "FILTER_HIGHER_TF_WEIGHT",
            float(profile_defaults["filter_higher_tf_weight"]),
        ),
        max_filter_penalty_cap=_get_env_float(
            "MAX_FILTER_PENALTY_CAP", float(profile_defaults["max_filter_penalty_cap"])
        ),
    )
    ...
    return Settings(
        ...
        stale_candle_warning_seconds=int(
            os.getenv("STALE_CANDLE_WARNING_SECONDS", "300")
        ),
