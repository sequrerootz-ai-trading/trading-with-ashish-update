...
def _is_strong_candle(candle: Candle):
    ...
    return (body / rng) > 0.5   # 🔥 was 0.6


# -----------------------------
# DIRECTION CHECK (MILD)
# -----------------------------
...
def _direction_check(signal: GeneratedSignal, data: SignalContext):
    ...
    # 🔥 relaxed momentum
    momentum = current_range > (prev_range * 1.05)
    ...
    volume_ok_for_breakout = True if current_volume <= 0 else current_volume >= (average_volume * 0.9)

    # 🔥 relaxed candle filters
    if current_range < 6:
        return False, "low_range"

    if abs(current_close - current_open) < (current_range * 0.2):
    ...
    bullish_follow_through = current_close >= prev_close
    bearish_follow_through = current_close <= prev_close

    if signal.signal == "BUY_CE":
        if breakout_up and momentum and volume_ok_for_breakout:
            return True, "call_breakout_ok"
        if bullish_follow_through and (strong_candle or trend_up):
            return True, "call_trend_ok"
        return False, "call_rejected"

    if signal.signal == "BUY_PE":
        if breakout_down and momentum and volume_ok_for_breakout:
            return True, "put_breakdown_ok"
        if bearish_follow_through and (strong_candle or not trend_up):
