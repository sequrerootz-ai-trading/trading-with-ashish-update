...
def _refresh_trade_extremes(active_trade, current_price: float, trade_manager: TradeManager):
    return _root_module()._refresh_trade_extremes(active_trade, current_price, trade_manager)
...
def _handle_partial_profit(symbol: str, active_trade, current_price: float, trade_manager: TradeManager, order_manager: OrderManager, market_type: str):
    return _root_module()._handle_partial_profit(symbol, active_trade, current_price, trade_manager, order_manager, market_type)
...
def manage_mcx_trade(symbol: str, trade_manager: TradeManager, premium_service, order_manager: OrderManager, candle_manager: CandleManager) -> bool:
    _reset_daily_state_if_needed("MCX")
    ...
    if active_trade.status == "PENDING_ENTRY":
        ...
        if active_trade.entry_low <= current_price <= active_trade.entry_high:
            updated_trade = _try_execute_entry_if_needed(symbol, active_trade, current_price, trade_manager, order_manager, "MCX")
    ...
    if _handle_live_stop_loss_completion(symbol, active_trade, current_price, trade_manager, order_manager, "MCX"):
        return True

    latest_candle = candle_manager.get_last_completed_candle(symbol)
    partial_trade = _handle_partial_profit(symbol, active_trade, current_price, trade_manager, order_manager, "MCX")
    ...
    if _should_time_exit(active_trade, current_price):
        ...
        if closed_trade is not None:
            _record_trade_result(closed_trade, exit_price, "MCX")
