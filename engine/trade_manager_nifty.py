...
def manage_nifty_trade(symbol: str, trade_manager: TradeManager, premium_service, order_manager: OrderManager, candle_manager: CandleManager) -> bool:
    ...
    if active_trade.status == "PENDING_ENTRY":
        ...
        if active_trade.entry_low <= current_price <= active_trade.entry_high:
            updated_trade = _try_execute_entry_if_needed(symbol, active_trade, current_price, trade_manager, order_manager, "EQUITY")
    ...
    if _handle_live_stop_loss_completion(symbol, active_trade, current_price, trade_manager, order_manager, "EQUITY"):
        return True

    latest_candle = candle_manager.get_last_completed_candle(symbol)
    partial_trade = _handle_partial_profit(symbol, active_trade, current_price, trade_manager, order_manager, "EQUITY")
