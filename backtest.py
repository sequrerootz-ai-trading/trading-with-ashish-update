...
class Backtester:
    def __init__(
        self, brokerage_per_trade: float = 40.0, slippage_pct: float = 0.01
    ) -> None:
    ...
    def run(
        ...
        candles: list[Candle],
        signal_fn: Callable[
            [list[Candle], int], tuple[str, float | None, float | None]
        ],
        lot_size: int = 1,
    ) -> BacktestResult:
        balance_curve = [0.0]
        ...
        for index in range(21, len(candles)):
            ...
            if (
                open_trade is None
                and signal in {"BUY_CE", "BUY_PE", "BUY", "SELL"}
                and entry_ref
                and stop_loss
            ):
            ...
            gross_pnl = (exit_price - open_trade["entry_price"]) * open_trade[
                "quantity"
            ]
...
def _build_metrics(
    trade_log: list[BacktestTrade], balance_curve: list[float]
) -> BacktestMetrics:
    ...
    return BacktestMetrics(
        ...
        profit_factor=(
            round(profit_factor, 2) if math.isfinite(profit_factor) else profit_factor
        ),
