...
class OrderManager:
    ...
    def _update_position(
        self, trading_symbol: str, quantity: int, price: float, side: str
    ) -> None:
        if side == "BUY":
        ...
        if side == "SELL":
            ...
            remaining_quantity = max(
                int(current_position.get("quantity", 0)) - quantity, 0
            )
    ...
    def calculate_quantity(
        self, last_price: float, capital_override: float | None = None
    ) -> int:
        if last_price <= 0:
            raise ValueError(
                "Last price must be greater than 0 for quantity calculation."
            )
        ...
        capital = (
            ...
            else self.settings.capital_per_trade
        )
    ...
    def calculate_risk_quantity(
        ...
        risk_amount = max(
            self.settings.account_balance * self.settings.risk_per_trade_pct, 1.0
        )
    ...
    def place_market_buy(
        ...
        if self._has_open_position(trading_symbol):
            logger.warning(
                "Position already exists for %s. Skipping BUY.", trading_symbol
            )
        ...
        entry_price = self._extract_average_price(
            entry_status, fallback_price=last_price
        )
        slippage = abs(entry_price - last_price)
        slippage_pct = slippage / max(last_price, 0.01)
        logger.info(
            "Slippage for %s: %.2f (%.2f%%)",
            trading_symbol,
            slippage,
            slippage_pct * 100,
        )
        ...
        live_stop_loss_price = self._round_to_tick(
            stop_loss_price
            if stop_loss_price is not None
            else self._calculate_stop_loss_price(entry_price)
        )
    ...
    def replace_stop_loss_order(
        ...
        if current_order_id:
            try:
                ...
                logger.warning(
                    "Unable to cancel stop-loss order %s: %s", current_order_id, exc
                )
    ...
    def check_order_status(self, order_id: str) -> dict[str, Any]:
        ...
        orders = self._with_retry(
            self.kite.orders, action_name=f"fetch order book for {order_id}"
        )
    ...
    def trail_stop_loss_to_price(
        ...
        if self.mode == "PAPER":
            ...
            logger.info(
                "[PAPER] Trailed stop loss %s to %.2f",
                trading_symbol,
                rounded_stop_loss,
            )
        ...
        logger.info(
            "[LIVE] Trailed stop loss %s to %.2f", trading_symbol, rounded_stop_loss
        )
        return rounded_stop_loss
    ...
    def _calculate_stop_loss_price(
        self, entry_price: float, percent: float | None = None
    ) -> float:
        stop_loss_percent = (
            percent if percent is not None else self.settings.stop_loss_percent
        )
    ...
    def _extract_average_price(order: dict[str, Any], fallback_price: float) -> float:
        average_price = (
            order.get("average_price") or order.get("price") or fallback_price
        )
